...
function StatusBadge({ status }) {
  const base = a.statusBadgeBase;
  if (status === "complete")
    return (
      <span className={`${base} ${a.statusBadgeComplete}`}>Completed</span>
    );
  if (status === "cancelled")
    return (
      <span className={`${base} ${a.statusBadgeCancelled}`}>Cancelled</span>
    );
  if (status === "confirmed")
    return (
      <span className={`${base} ${a.statusBadgeConfirmed}`}>Confirmed</span>
    );
  if (status === "rescheduled")
    return (
      <span className={`${base} ${a.statusBadgeRescheduled}`}>Rescheduled</span>
    );
  return <span className={`${base} ${a.statusBadgePending}`}>Pending</span>;
}
  ...
  if (appointment.status === "rescheduled") {
    return (
      <select
        ...
        className={`${a.statusSelect} ${
          terminal ? a.statusSelectDisabled : a.statusSelectEnabled
        }`}
      ...
      {!editing ? (
        <div className="flex justify-end">
          <button
            ...
            className={`${a.rescheduleButton} ${
              terminal ? a.rescheduleButtonDisabled : a.rescheduleButtonEnabled
            }`}
          >
            Reschedule
          </button>
        ...
        <div className={a.rescheduleForm}>
          <input
            type="date"
            value={date}
            min={minDate}
            onChange={(e) => setDate(e.target.value)}
            className={a.dateInput}
          />
          <input
            type="time"
            value={time}
          ...
          />
          <div className={a.rescheduleButtons}>
            <button onClick={save} className={a.saveButton}>
              Save
            </button>
            <button onClick={cancel} className={a.cancelButton}>
              Cancel
            </button>
          </div>
        </div>
    ...
    <div className={a.pageContainer}>
      <div className={a.contentWrapper}>
        <div className={a.headerContainer}>
          <div>
            <h1 className={a.headerTitle}>All Appointments</h1>
